...
for line in sys.stdin:
    ...
    if line == 'INITIALIZE':
        ...
        try:
            # Best-effort symbol select; some brokers return False even for
            # already-visible symbols, so we proceed regardless.
        ...
        date_from_str = sys.stdin.readline().strip()
        count = int(sys.stdin.readline().strip())
        flags = int(sys.stdin.readline().strip())
        ...
        try:
            ...
            if ticks is None:
                error_info = mt5.last_error()
                error_msg = f"error:No tick data for {symbol}, MT5 error: {error_info}"
                send(error_msg)
                continue
            ...
            error_msg = f"error:Exception in COPY_TICKS_FROM: {str(e)}"
            send(error_msg)
        ...
        try:
            ...
            error_msg = f"error:Exception in COPY_TICKS_RANGE: {str(e)}"
            send(error_msg)
