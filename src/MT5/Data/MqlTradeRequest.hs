...
data MqlTradeRequest = MqlTradeRequest
  ...
  , trReqOrder       :: Int                -- ^ Order ticket. Required for modifying pending orders
  ...
  , trReqVolume      :: Double             -- ^ Requested volume of a deal in lots. A real volume when making a deal
                                           -- depends on an order execution type.
  , trReqPrice       :: Double             -- ^ Price at which an order should be executed. The price is not set in
  ...
  , trReqStoplimit   :: Double             -- ^ A price a pending Limit order is set at when the price reaches the
                                           -- 'price' value (this condition is mandatory). The pending order is not
                                           -- passed to the trading system until that moment
  , trReqSl          :: Double             -- ^ A price a Stop Loss order is activated at when the price moves in an
                                           -- unfavorable direction
  , trReqTp          :: Double             -- ^ A price a Take Profit order is activated at when the price moves in a
                                           -- favorable direction
  ...
  , trReqPosition    :: Int                -- ^ Position ticket. Fill it when changing and closing a position for its
                                           -- clear identification. Usually, it is the same as the ticket of the order
                                           -- that opened the position.
  , trReqPositionBy  :: Int                -- ^ Opposite position ticket. It is used when closing a position by an
