#!/bin/bash
#
# Test script for MT5 REST API Bridge - Close Partial Position
# Tests position_close_partial action
#
...
echo -e "${BLUE}=== MT5 REST API Bridge - Close Partial Position Test ===${NC}\n"
...
echo -e "${YELLOW}Usage: $0 [ticket_number] [volume]${NC}\n"

# Clean up old response
rm -f "$RESPONSE_FILE"

# Create request
echo -e "${BLUE}Requesting to partially close position $TICKET (volume: $VOLUME)...${NC}"
...
if [ "$SUCCESS" = "true" ]; then
    ...
    VOLUME_CLOSED=$(cat "$RESPONSE_FILE" | jq -r '.volume_closed // "N/A"')
    PRICE=$(cat "$RESPONSE_FILE" | jq -r '.price // "N/A"')
    
    echo ""
    echo -e "${GREEN}✓ Position partially closed successfully!${NC}"
    echo -e "${BLUE}Return Code:${NC} $RETCODE"
    echo -e "${BLUE}Deal:${NC} $DEAL"
    echo -e "${BLUE}Order:${NC} $ORDER"
    echo -e "${BLUE}Volume Closed:${NC} $VOLUME_CLOSED"
    ...
    echo ""
    echo -e "${RED}✗ Partial position close failed${NC}"
