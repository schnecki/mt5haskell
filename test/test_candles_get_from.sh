#!/bin/bash
#
# Test script for MT5 REST API Bridge - Get Candles (From Mode)
# Tests candles_get action with from mode
#
...
echo -e "${BLUE}=== MT5 REST API Bridge - Get Candles (From Mode) Test ===${NC}\n"
...
echo -e "${YELLOW}Usage: $0 [symbol] [timeframe] [start_timestamp] [count]${NC}\n"

# Clean up old response
rm -f "$RESPONSE_FILE"

# Create request
echo -e "${BLUE}Requesting $COUNT candles from $(date -d @$START_TIME)...${NC}"
